...
def solution(today, terms, privacies):
    ...
    today_ = yy*12*28 + mm*28+dd
    
    terms_list={}
    for i in range(len(terms)):
        terms_type_1, month = terms[i].split(" ")
        terms_list[terms_type_1] = int(month)

    privacies_list = []
    for i in range(len(privacies)):
        info_date, terms_type = privacies[i].split(" ")
        y, m, d = map(int, info_date.split('.'))
        d = m *28 + y*12*28 + terms_list[terms_type]*28 +d
        privacies_list.append([i+1, d])
